...
| `src/model/relief.ts` | **Shared primitives for the image-relief types** (litho + fan), a primitive layer like `csg.ts`: the async image decode cache (`prepareImage`/`getGray`/`sampleLum`), `imageFileToDataURL`, `heightfieldMesh()` (closed manifold-by-construction heightmap slab), `ditherGrid()` (layer-step quantisation + error diffusion), `greyLevels()`. Knows nothing about panels or blades. |
...
Note the heightmap mesh, the dithering and the image cache all live in **`relief.ts`** (shared with the fan) — `litho.ts` keeps only the panel's shape and placement. `relief.ts` is a primitive layer like `csg.ts`: it must stay ignorant of panels and blades.
...
- **The cell budget is shared across blades** (`MAX_CELLS` total, not per blade), so blade count trades against detail instead of building an unsliceable mesh: 9 blades ≈ 208k triangles / ~0.4mm effective pitch / ~800ms, and 24 blades stays at ~223k.
