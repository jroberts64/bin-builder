...
One watertight mesh, modelled Y-up standing on Y=0 like a framed picture (width X, image height Y, thickness Z; flat back on z=0, relief toward +Z). The exporter's Y→Z rotation keeps it standing in the slicer — the correct lithophane print orientation (vertical, so layer lines don't blur the image). Dark = thick (defaults 0.8–3mm), `invert` flips; backlight to reveal the picture.
