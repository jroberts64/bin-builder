...
- **flat** (default) — panel on its back, relief up. Tone is stacked in layers, so it's quantised by layer height (~12 greys over 0.8–3mm at 0.2mm layers, ~23 at 0.1mm); picture-plane detail is limited by extrusion width. Fast (15 layers), no brim, zero overhangs; smooth gradients can band. Buying tone is cheap here — halving the layer height doubles the greys and the part is only 3mm thick.
