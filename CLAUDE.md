...
| `src/model/fan.ts` | `FanModel`, `defaultFan()`, `fanOuterSize()`, `buildFan()` → `{ blades, size }` (**one mesh per blade**). Blade silhouette, the image-across-the-open-fan mapping, `fanFrame()`/`overlapRadius()`/`reliefStartRadius()`/`fanLayout()`, and the two placements (`orientFanForPreview`, `placeFanForPrint`). Built on `relief.ts`. |
...
- **fan** → `buildFan` (`fan.ts`), **N** meshes, one per blade. Same heightmap-not-CSG approach as the panel (see "Lithophane fans" below).
...
- **The pivot zone must be a flat plate.** Inside `reliefStartRadius(m)` thickness is exactly `hubThickness` (blended out over `HUB_BLEND`), because blades stack and pivot against each other there — relief would make them bind. When dithering, the hub cells are **re-flattened after** `ditherGrid`, or error diffusing in from the relief boundary leaves a layer of speckle on the very faces that have to slide. Verify: every vertex inside the radius is exactly `hubThickness`.
...
- **`fanLayout`** wraps the row at `LAYOUT_MAX_W` and then evens the rows out (9 blades → 5 + 4, 132×215mm), so a full fan still fits a plate. Verified no two blade bboxes overlap.
...
- STL is **one combined mesh of the whole plate** (a zip of a dozen near-identical blade files would be useless — they all print together anyway); 3MF is one `<object>` per blade in its plate position.
