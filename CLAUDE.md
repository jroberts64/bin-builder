...
- Contrast is the **ratio** `maxThickness / minThickness`, not the difference — so scaling the whole range down thins the blade *and the hub stack* without costing contrast. What it does cost is printable grey levels (`greyLevels`), which a finer layer height buys back; a thinner blade prints fast enough to afford one.
